  ...
  const buildDataBundle = () => {
    try {
      ...
      const lineRows = lines.map((line) => {
        ...
        // Workforce - simulate SQL logic where present is operators who performed specific operations
        const activeEmployees = Math.floor(85 + r() * 35); // per line (from hangerline_emp)
        // Present employees: simulate count of distinct odp_em_key who worked on filtered operations/lots
        const presentEmployees = Math.floor(activeEmployees * (0.7 + r() * 0.3)); // 70%..100% of active
  ...
  const buildFallbackBundle = () => {
    return {
      ...
      summary: {
        ...
        attendancePct: 1220 > 0 ? (1148 / 1220) * 100 : 0,
      },
      breakdownCategories: {
        cats: [
          { name: 'Mechanical', color: 'rgba(239,68,68,.85)' },
    ...
    try {
      ...
      const lineRows = filteredLines.map((row) => {
        if (shift === 'Day') {
          ...
          const newActive = Math.floor(row.workforce.activeEmployees * 0.55);
          // Simulate present as operators who worked on day shift operations
          const newPresent = Math.floor(newActive * (0.75 + seededRand(100 + row.line.charCodeAt(0))() * 0.25)); // 75-100% for day
          return {
            ...row,
            ...
            workforce: {
              activeEmployees: newActive,
              presentEmployees: newPresent,
            },
          };
        }
        if (shift === 'Night') {
          const ratio = row.shift.night / Math.max(1, row.shift.day + row.shift.night);
          const newActive = Math.floor(row.workforce.activeEmployees * 0.45);
          // Simulate present as operators who worked on night shift operations
          const newPresent = Math.floor(newActive * (0.65 + seededRand(200 + row.line.charCodeAt(0))() * 0.25)); // 65-90% for night
          return {
            ...row,
            offloading: Math.floor(row.offloading * ratio),
            loading: Math.floor(row.loading * ratio),
